...
async def maintenance_dashboard(db: Session = Depends(get_db)):
    """
...
async def get_history(component_id: int, db: Session = Depends(get_db)):
    """
...
async def delete_component(maintenance_id: int, db: Session = Depends(get_db)):
    """
